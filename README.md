...
**Когда нужно пересобирать Docker образы (`make docker-build`):**

Пересборка образа требуется в следующих случаях:

1. **Первый запуск проекта** - обязательно собрать образ перед запуском
2. **Изменения в зависимостях Python** - при изменении `pyproject.toml` или `poetry.lock`:
   - Добавление новых пакетов
   - Обновление версий существующих пакетов
   - Удаление зависимостей
3. **Изменения в Dockerfile** - при любых изменениях в `Dockerfile` или `Dockerfile.dev`:
   - Изменение базового образа Python
   - Добавление системных пакетов (apt-get)
   - Изменение версии Poetry
   - Изменение структуры установки зависимостей
4. **Изменения в системных зависимостях** - при необходимости установки новых системных библиотек

**Когда НЕ нужно пересобирать образ:**

- **Изменения в коде приложения** - благодаря volume mount (`.:/app` в `docker-compose.yml`) изменения в Python коде применяются автоматически без пересборки
- **Изменения в переменных окружения** - достаточно перезапустить контейнеры (`make docker-restart`)
- **Обычная разработка** - после первой сборки можно просто использовать `make docker-up` и `make docker-restart`

**Рекомендация:** После изменений в зависимостях или Dockerfile выполните:

```bash
make docker-build    # Пересобрать образ с новыми зависимостями
make docker-up        # Запустить обновленные контейнеры
```
